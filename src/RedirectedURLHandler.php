...
class RedirectedURLHandler extends Extension
{
    ...
    public function onBeforeHTTPError404(HTTPRequest $request)
    {
        ...
        $potentials = RedirectedURL::get()->filter(['FromBase' => '/' . $SQL_base])->sort('FromQuerystring ASC');
        ...
        for ($pos = count($baseparts) - 1; $pos >= 0; $pos--) {
            ...
            $basepots = RedirectedURL::get()->filter(['FromBase' => '/' . $basepart])->sort('FromQuerystring ASC');
